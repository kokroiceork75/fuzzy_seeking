...
/////////////////////  設定後件部、中心點與寬度的參數，用於資料更新  ///////////////////
 //限制後件部的範圍 
#define   _conq_max_Kp  15.6      /*  後件部最大值  */
#define   _conq_max_K  5       /*  後件部最大值  */
#define   _Turn_max_Kp  5      /*  後件部最大值  */
#define   _conq_min_Kp  0      /*  後件部最小值  */
#define   _conq_min_K  -5     /*  後件部最小值  */
#define   _Turn_min_K  -5     /*  後件部最小值  */
 // 限制寬度的範圍 
...
///////////////////  速度項目  /////////////////////{
#define left_wheel_speed 15.6  /* 左輪輪速 */
#define right_wheel_speed  15.6   /* 右輪輪速  */
...
//指定條件的重置功能 
  ...
  const double INITIAL[3] = { 2.24607, 0.0891353, -16.6811 };  //初
...
for (i=1; i<=popsize; i++ )
{
 for (jj=1; jj<=_rule_number; jj++)
  {			
   ...
   parent_pop[i].xreal[jj*_mem_length-_out_varl+1] =  5*((2.0*(double)rand()/RAND_MAX)-1); //Kp 的 -5~5的隨機值
   parent_pop[i].xreal[jj*_mem_length-_out_varl+2] =  5*((2.0*(double)rand()/RAND_MAX)-1); //Kd 的 -5~5的隨機值 
   parent_pop[i].xreal[jj*_mem_length-_out_varl+3] =  5*((2.0*(double)rand()/RAND_MAX)-1); //Ki 的 -5~5的隨機值 
   parent_pop[i].xreal[jj*_mem_length-_out_varl+4] =  (15.6*(double)rand()/RAND_MAX); //Kp 的 3~15.6的隨機值
   parent_pop[i].xreal[jj*_mem_length-_out_varl+5] =  5*((2.0*(double)rand()/RAND_MAX)-1); //Kd 的 -8~8的隨機值
   parent_pop[i].xreal[jj*_mem_length-_out_varl+6] =  5*((2.0*(double)rand()/RAND_MAX)-1);//Kd 的 -8~8的隨機值
...
in_mu_2 =  1. ;
     ...
     i= _in_varl;
    {
          ...
          //  printf("phi(in[i] , fuzzy_real[ber][(_rule-1)*_mem_length+i*2-1] , fuzzy_real[ber][(_rule-1)*_mem_length+i*2] )=%lf\n",phi(in[i] , fuzzy_real[ber][(_rule-1)*_mem_length+i*2-1] , fuzzy_real[ber][(_rule-1)*_mem_length+i*2] ));
     }  
...
double den[_out_varl+1] ,   //分母項
 ...
 for (j=1; j<=_out_varl-2; j++)
 {
  ...
  // printf("\n前/後件部:%lf\t%lf\n", _Rule[i].con[j],_Rule[i].in_mu);
  // printf("\n前/後件部:%lf\n", _Rule[i].con[j]);

 // printf("num[%d]=%lf \t den[%d]=%lf\n ",j, num[j],j,  den[j]);
   }
 ...
 for (j=4; j<=_out_varl; j++)
 {
  ...
  num[j] = 0. ;  //初始化歸零
   ...
   num[j] = num[j] + _Rule[i].in_mu_2 * _Rule[i].con[j];  //num為sigma的寫法
   den[j] = den[j] + _Rule[i].in_mu_2 ;   //den為sigma的寫法
  //printf("num[%d]=%lf \t den[%d]=%lf\n ",j, num[j],j,  den[j]);
  // printf("\n前/後件部:%lf\t%lf\n", _Rule[i].con[j],_Rule[i].in_mu);
  // printf("\n前/後件部:%lf\n", _Rule[i].con[j]);
